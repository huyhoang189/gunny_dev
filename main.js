...
//region updater
let updateUrl =
  config.host + `/api/update/${process.platform}/${app.getVersion()}`;
    ...
    .get(url, function (response) {
      ...
      response.on("end", () => {
        try {
          ...
          if (update != "0") {
            if (msg != 1) {
              var click = dialog.showMessageBoxSync(
                BrowserWindow.getFocusedWindow(),
                {
                  ...
                  message:
                    ...
                    update +
                    "), tải về và cập nhật ngay bây giờ?",
                  type: "question",
                }
              );
              if (click == 1) {
          ...
          if (msg == 1) {
            dialog.showMessageBoxSync(BrowserWindow.getFocusedWindow(), {
              ...
              title: "Cảnh báo",
              message:
                "Không thể kết nối máy chủ, vui lòng liên hệ quản trị viên để được hỗ trợ!",
              type: "error",
            });
          }
        }
...
ipcMain.on("download-update", (event, info) => {
  download(BrowserWindow.getFocusedWindow(), downloadUpdateUrl, {
    ...
    onCompleted: function (obj) {
      var clicked = dialog.showMessageBoxSync(
        BrowserWindow.getFocusedWindow(),
        {
          ...
          title: "Thông báo",
          message:
            "Bản cập nhật đã tải xong, thoát game và cập nhật ngay bây giờ?",
          type: "question",
        }
      );
      if (clicked === 1) {
    ...
    updateWindow.webContents.send("download-complete", dl.getSavePath())
  );
});
...
ipcMain.on("tfa-exchange-validation", (evt, arg) => {
  var window = windowIndexes[arg.windowIndex];
  if (window !== null) {
    TFAExchangeValidationWindow = createTFAExchangeValidationWindow(
      true,
      window
    );
  }
