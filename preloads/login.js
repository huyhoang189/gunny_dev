...
window.addEventListener("DOMContentLoaded", () => {
  ...
  var logoImage = new Image(430, 200);
  canvas4 = document.getElementById("logoCanvas");
  context4 = canvas4.getContext("2d");
  logoImage.onload = function () {
    context4.drawImage(logoImage, 0, 0, 430, 200);
  };
  ...
  registerButtonTab.addEventListener("click", function () {
    shell.openExternal("http://gunhuyenthoai.me");
  });
  ...
  var contextmenuItems = document.getElementsByClassName(
    "recentAccountContextMenuItem"
  );
...
//       if (err) {
//         ipcRenderer.send("errorbox", ["login", "Cảnh báo", err.message]);
//       }
//     });
//   }
// };
...
function loginCallback(response) {
  ...
  if (response.success == true) {
    ...
    ipcRenderer.send("tfa-login-validation", [
      usernameInput.value,
      passwordInput.value,
      "login",
    ]);
  } else {
    ipcRenderer.send("errorbox", ["login", "Cảnh báo", response.message]);
  }
}
...
function registerCallback(response, request) {
  ...
  if (!empty(response.success) && response.success === true) {
    ipcRenderer.send("infobox", ["login", "Thông báo", response.message]);
    ...
    ipcRenderer.send("errorbox", ["login", "Cảnh báo", message]);
  }
}
...
window.addEventListener("click", function (e) {
  ...
  if (
    target.localName == "td" &&
    target.parentNode.className == "recentAccount"
  ) {
    var usernameInput = document.getElementById("lgusername");
    var passwordInput = document.getElementById("lgpassword");
...
window.addEventListener("mouseup", function (e) {
  // var recentAccountContextMenu = document.getElementById(
  //   "recentAccountContextMenu"
  // );
  // if (e.button == 2) {
  //   //right click
  //   var target = e.target;
